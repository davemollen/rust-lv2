...
//! # Example
//!
//! This following plugin log an error message each time the input control is
//! switched on.
//!
//! Note: Logging an error is not real-time safe. This following plugin should
//! not ask `hardRTCapable` in its .ttl files.
//!
//! ```
//! use core::any::Any;
...
/// URID marker. The corresponding URID is used to log an error message on the
/// host.
pub struct Error;
...
/// URID marker. The corresponding URID is used to log an informative message on
/// the host.
pub struct Note;
...
/// URID marker. The corresponding URID is used to log a debugging trace on the
/// host.
pub struct Trace;
...
/// URID marker. The corresponding URID is used to log a warning message on the
/// host.
pub struct Warning;
...
/// Trait for URID marker. URID with a marker implementing this can be used to
/// indicate the logging level.
///
/// Plugin implementers can implement this on custom URID marker to define and
/// use additional logging level.
//
...
impl<'a> Log<'a> {
    ...
    /// # Real-Time safety
    ///
    /// This function is not real-time safe. Except for logging debugging trace,
    /// it should not be used in real-time context. That means
    /// `HardRTCapable` plugins should not call this function in
    /// their `run()` context.
